 ...
 * 使用例:
 * 
 * // BrowserPage.tsx または useHighlighter.ts で
 * import { WebViewDebugger } from '../utils/webview-debug';
 * 
 * const debugger = new WebViewDebugger(webviewEl, tabId);
 * 
 * // 完全な診断を実行
 * debugger.runFullDiagnostics();
 * 
 * // 特定のテストだけ実行
 * debugger.checkHighlighterState();
 * debugger.testOverlay();
 * debugger.testModeChange('element');
 * 
 * // 継続的なモニタリング開始
 * const stopMonitoring = debugger.startHealthMonitoring(5000);
 * // 停止する場合
 * stopMonitoring();
 */